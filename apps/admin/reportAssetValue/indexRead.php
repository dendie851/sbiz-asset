<?php 
	...
	if(count($categoryAssetId) > 0) {
		...
		if(strlen($categoryAssetIdValue) > 0) {
			$query = "select id,name 
				from category
				where is_delete = '0'
				and id not in ($categoryAssetIdValue)
				order by name";
		} else {
			$query = "select id,name 
				from category
		...
		$dataCategoryInfo = mysqli_query($con, $query) or die(mysqli_error($con));		 
	} else {
	...
	$query = "select id,name 
		...
		where is_delete = '0'
		order by name";
	$dataCategoryAsset = mysqli_query($con, $query) or die(mysqli_error($con));
	...
	$query = "select id,name
		...
		order by name";
	$tmp = mysqli_query($con, $query) or die(mysqli_error($con));
	$printDataDepartement = mysqli_fetch_array($tmp);
