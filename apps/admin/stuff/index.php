				...
				<td>
					<select name="categoryId" style="width:180px">
						...
						<?php while ($val = mysqli_fetch_array($dataCategory)): ?>
							<option value="<?php echo $val['id'] ?>" <?php echo $val['id'] == (isset($_REQUEST['categoryId']) ? $_REQUEST['categoryId'] : '') ? 'selected' : '' ?>>
								<?php echo $val['name'] ?>
							</option>
						<?php endwhile; ?>
					</select>
		...
		<table width="100%" border="1">
			<thead>
				<tr>
					...
					<th align="center" width="15%">UKURAN</th>
					<th></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						...
						<td align="center">
							...
							<?php if (strlen($val['foto_thumb']) > 0): ?>
								<!-- http://192.168.1.21/asset/admin/asset/foto/<?php echo $val['foto'] ?> -->
						...
						<td align="center">
							...
							<small style="font-size:9px; padding-left:5px">(
						...
						<td align="center">
							<input type="button" value="NOMOR SERI ASSET"
								onclick="window.location='../stuffSeries/index.php?id=<?php echo $val['id'] ?>'" />
							<input type="button" value="EDIT"
								onclick="window.location='edit.php?id=<?php echo $val['id'] ?>'" />
							<input type="button" value="HAPUS"
								onclick="confirm('Anda yakin akan menghapus ?') ? window.location='delete.php?id=<?php echo $val['id'] ?>' : false" />
						</td>
					</tr>
					<?php $i++; ?>
				<?php endwhile; ?>
