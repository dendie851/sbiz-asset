...
<?php if (isset($_GET['msg'])): ?>
	<div class="info">
		...
		<h5>Jumlah data yang terkena pembatalan penyusutan
			<?php echo $_GET['jumlahData'] ?> data
		</h5>
	</div>
<?php endif ?>
			...
			<tbody>
				...
				<?php while ($val = mysqli_fetch_array($data)): ?>
					<tr>
						...
						<td align="center">
							<?php echo $val['date_name'] ?>
						</td>
						<td align="center">
							<a href="detail.php?id=<?php echo $val['id'] ?>">DETAIL</a>
							<?php if ($_SESSION['positionId'] != '3'): ?>
								<input type="button" value="BATALKAN"
									onclick="confirm('Anda yakin akan membatalkan ?') ? window.location='cancel.php?id=<?php echo $val['id'] ?>' : false" />
							<?php endif; ?>
						</td>
						<?php $i++; ?>
					<?php endwhile; ?>
