	...
	<table width="100%">
		<tr>
			...
			<td style="text-align:right">
				<input type="button"
					onclick="window.open('excel.php?categoryAssetId=<?php echo implode(',', $categoryAssetId) ?>&dateBuy=<?php echo $dateBuy ?>&fundId=<?php echo $fundId ?>&departementId=<?php echo $departementId ?>&referencePrice=<?php echo $referencePrice ?>&condition=<?php echo str_replace("'", '', implode(',', $_REQUEST['condition'])) ?>')"
					value="PRINT TO EXCEL">
				<input type="button"
					onclick="window.open('print.php?categoryAssetId=<?php echo implode(',', $categoryAssetId) ?>&dateBuy=<?php echo $dateBuy ?>&fundId=<?php echo $fundId ?>&departementId=<?php echo $departementId ?>&referencePrice=<?php echo $referencePrice ?>&condition=<?php echo str_replace("'", '', implode(',', $_REQUEST['condition'])) ?>')"
					value="PRINT">
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<select name="condition[]" style="width:245px; height:70px" multiple>
					<option value="'0'" <?php echo in_array("'0'", $_REQUEST['condition']) ? 'selected' : '' ?>>RUSAK
					</option>
					<option value="'1'" <?php echo in_array("'1'", $_REQUEST['condition']) ? 'selected' : '' ?>>BAIK
					</option>
					<option value="'2'" <?php echo in_array("'2'", $_REQUEST['condition']) ? 'selected' : '' ?>>SETENGAH
						BAIK</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td align="top">
				<select name="categoryAssetId[]" style="width:245px; height:200px" multiple>
					<?php while ($val = mysql_fetch_array($dataCategoryAsset)): ?>
						<option value="<?php echo $val['id'] ?>" <?php echo $val['id'] == (isset($_REQUEST['categoryAssetId']) ? in_array($val['id'], $_REQUEST['categoryAssetId']) : '') ? 'selected' : '' ?>>
							<?php echo $val['name'] ?>
						</option>
					<?php endwhile; ?>
				</select><br />
				...
				<?php if ($dataCountCategoryAsset['jml'] > 0): ?>
					...
					*/
					...
					$query = "SELECT 
								...
								FROM asset as a
								LEFT JOIN asset_series as ase ON a.id = ase.asset_id 
									AND ase.is_delete = '0' 
									AND ase.is_remove = '0'
								LEFT JOIN asset_history as ah ON ah.asset_series_id = ase.id 
									AND ah.type = '0' 
									AND ah.date >= '$dateBuy'
								WHERE a.is_delete = '0' 
												...
												<?php while ($valAsset = mysql_fetch_array($dataAsset)): ?>
													<?php if ($valAsset['jml'] > 0): ?>
														<tr>
															...
															<td align="center">
																<div class="button">
																	<a href="detail.php?dateBuy=<?php echo $dateBuy ?>&assetId=<?php echo $valAsset['id'] ?>&fundId=<?php echo $fundId ?>&departementId=<?php echo $departementId ?>&condition=<?php echo str_replace("'", '', implode(',', $_REQUEST['condition'])) ?>"
																		data-title="DETAIL" data-width="600"
																		data-height="550"><?php echo $valAsset['jml'] ?></a>
																</div>
