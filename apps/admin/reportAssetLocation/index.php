					...
					<select name="locationId" style="width:245px">
						...
						<?php foreach ($dataLocationCmb as $val): ?>
							<option value="<?php echo $val['id'] ?>" <?php echo $val['id'] == (isset($_REQUEST['locationId']) ? $_REQUEST['locationId'] : (isset($data['locationId']) ? $data['locationId'] : '')) ? 'selected' : '' ?>>
								<?php echo $val['name'] ?> 	<?php echo $val['alias'] ?>
							</option>
						<?php endforeach; ?>
					</select>
				</td>
		...
		<table width="100%">
			<tr>
				<td style="text-align:right">
					<input type="button" onclick="window.open('excel.php?locationId=<?php echo $_REQUEST['locationId'] ?>')"
						value="PRINT TO EXCEL">
					<input type="button" onclick="window.open('print.php?locationId=<?php echo $_REQUEST['locationId'] ?>')"
						value="PRINT">
				</td>
			</tr>
		</table>
							...
							<tr>
								...
								<?php else: ?>
									...
									<td align="left">
										...
										<p>
											<small>[ <a href="#"
													onclick="window.open('printCard.php?locationId=<?php echo $val['location_id'] ?>')">PRINT
													KARTU ASSET</a> ]</small><br />
											<small>[ <a href="#"
													onclick="window.open('printCardExcel.php?locationId=<?php echo $val['location_id'] ?>')">EXPORT
													KARTU ASSET to Excel</a> ]</small>
										</p>
									</td>
									<?php $i++; ?>
								<?php endif; ?>
