...
<fieldset>
	...
	<table width="100%">
		<tr>
			...
			<td>: <b>
					<?php echo $assetSeriNomor ?>
				</b></td>
		</tr>
		<tr>
			<td>NAMA ASSET</td>
			<td>: <b>
					<?php echo $dataAsset['name'] ?> <small>(
						<?php echo $dataAsset['category_name'] ?>)
					</small>
				</b></td>
		</tr>
	</table>
				...
				<tbody>
					...
					<?php while ($val = mysqli_fetch_array($data)): ?>
						<tr>
							...
							<td align="center">
								<?php echo $val['decription'] ?>
							</td>
							<?php if (in_array($_SESSION['positionId'], array(1, 2))): ?>
								<td align="center"><input type="button" value="HAPUS"
										onclick="confirm('Anda yakin akan menghapus ?') ? window.location='delete.php?assetId=<?php echo $id ?>&id=<?php echo $val['id'] ?>' : false" />
								</td>
							<?php endif; ?>
						</tr>
						<?php $i++; ?>
